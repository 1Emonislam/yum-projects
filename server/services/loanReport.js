...
const getS3UploadPresignedUrl = require('./getS3UploadPresignedUrl')
...
const generateInstallments = ({
  ...
  const dueDates = generateDueDates({
    initialDueDate: moment(startDate).tz(process.env.TIMEZONE).endOf('day'),
...
module.exports = async function (db, user = null) {
  ...
  for (let [index, loan] of loans.entries()) {
    ...
    const group = groups.find(
      group => group._id.toString() === client.clientGroupId.toString()
    )
    ...
    let expectedRealization = loan.installments.reduce(
      (acc2, installment = {}) => {
        ...
        if (moment(due).tz(timezone).isAfter(moment(), 'day')) {
          // include overpayments (difference between total and target on last installments)
          return acc2 + (total - target)
        }
    ...
    if (client && client.status !== 'inactive') {
      worksheet.addRow({
        ...
        disbursementDate: moment(loan.disbursementAt)
          .tz(timezone)
          .format('DD/MM/YYYY'),
        disbursedAmount,
  ...
  const { signedUrl: url, previewUrl: reportUrl } =
    await getS3UploadPresignedUrl({ filename, filetype, type: 'reports/loan' })
